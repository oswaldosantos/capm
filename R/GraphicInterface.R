...
#' @param set.func string to select a graphic interface for a set of functions to achieve a specific task. \code{SelectSampling Units} creates a graphic interface to select sampling units for pilot or final survey designs. \code{CalculateSampleSize} creates a graphic interface to calculate sample size and composition. The graphic interface created by \code{SurveyAnalysis} support functionality to analyise survey data. \code{SolveIASA} creates an interface to simulate population dynamics and to assess population-based interventions.
