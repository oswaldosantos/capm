...
#' @param total value indicating the number of sampling units in the population. This is an alternative to \code{psu.ssu}, intended for non complex designs.
...
siss <- function(psu.ssu = NULL, ssu = NULL, total = NULL, write = FALSE, ...) {
  if (!is.null(total)) {
    int <- total / ssu
    k = sample(int, 1)
    Ssu = rep(NA, ssu)
