...
#' @param estimates output from the \code{\link{SummarySurvey}} function (see detials).
...
#' design <- DesignSurvey(psu.ssu, Sample, psu.col = 2, 
#'                        ssu.col = 1)
#' 
...
#' estimates <- SummarySurvey(design, variables, rnd = 3)
#' 
#' ## Then: 
...
#' ## Set ranges based on estimated confidence limits.
#' 
#' # Prepare the data (see SummarySurvey examples).
...
#' estimates <- SummarySurvey(design, variables, rnd = 3)
#' estimates.f <- SummarySurvey(design.f, variables.f, rnd = 3)
#' estimates.m <- SummarySurvey(design.m, variables.m, rnd = 3)
#' 
#' ## Then:
#' # See which rows contain the estimates of interest.
#' data.frame(rownames(estimates))
...
#' par.rg.SolveIASA <- SetRanges(pars = rg.SolveIASA,
#'                               estimates = estimates,
#'                               rowpars = new.est)
