...
#' # Guia rápido de programação com o capm 0.5 em R versão 3.1.0
...
#'  * santos.shx
#'  * survey.data.csv
#'  No enlace anterior também encontrará um arquivo chamado guia_rapido_de_programacao_capm_0.5.R com os códigos deste guia.
...
#' O arquivo contém exatamente a informação que precisamos para amostrar UPM com probabilidade proporcional a seus tamanhos e com reposição. Se o argumento `write` de `SamplePPS` é definido como TRUE, as UPM selecionadas serão salvas num arquivo csv, que pode ser visto num software de planilhas. O resultado terá tantas linhas como UPM selecionadas. Lembre que a mesma UPM pode ser selecionada mais de uma vez porque a amostragem é com reposição.<br>  
...
# Se aparece o erro '503 Service Unavailable' tente mais tarde para ver se 
# os servidores OSM voltam a funcionar (ver página de ajuda de get_openstreetmap).
...
#' Qualquer que seja o método usado para produzir os mapas, devemos traçar um percurso no mapa de cada UPM selecionada, de forma que todas as ruas sejam percorridas. Podemos fixar um domicílio num ponto arbitrário (ex. localização inferior esquerda) como o primeiro domicílio e a partir do mesmo, percorrer o percurso contando domicílios (incluindo as duas calçdas dos fragmentos de ruas contidos totalmente na UPM) e entrevistando os que foram selecionados.<br>  
...
#' Depois de ter definido a amostra final, imaginemos que fomos a visitar todos os domicílios selecionados e que os dados coletados foram registrados num arquivo chamado `survey.data.csv`.<br>  
...
#' Definir o tipo de estimativa para cada variável é fácil. Aspas vazias exclunuma variável do processo de estimação.
...
#' As pirâmides populacionais resumem a composição básica de uma população e como mínimo são construídas usando as variáveis idade e sexo, mas podem ser condicionadas a uma terceira variável categórica. Os dados a serem usados devem ter cada variável numa coluna separada e essas colunas devem ser especificadas nos respectivos argumentos da função. Para que as etiquetas da figura fiquem em português, devemos transformar os `"labels"` das variáveis `"sex"` e `"sterilized"`, bem como o nome desta última.
